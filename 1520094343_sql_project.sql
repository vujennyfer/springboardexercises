...
The facilities that charge a fee:
Tennis Court 1
Tennis Court 2
Massage Room 1
Massage Room 2
...
the member name. */
SELECT CONCAT( firstname, " ", surname ) AS member_fullname, F.name AS court_name
FROM Bookings B
...
SELECT sub.facility AS facility, sub.member AS member, sub.cost AS cost
FROM (
